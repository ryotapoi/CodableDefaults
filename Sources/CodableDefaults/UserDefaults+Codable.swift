...
extension UserDefaults {
    ...
    public func value<Value: Codable>(type: Value.Type = Value.self, forKey key: String, default defaultValue: Value) -> Value {
    ...
    /// Retrieves an NSObject value conforming to NSSecureCoding from UserDefaults.
    ///
    /// If the key does not exist or decoding fails, the specified default value is returned.
    ///
    /// - Parameters:
    ...
    ///   - defaultValue: The default value to return if the key does not exist.
    /// - Returns: The value of the specified type, or the defaultValue if decoding fails.
    public func value<Value: NSObject & NSSecureCoding>(type: Value.Type, forKey key: String, default defaultValue: Value) -> Value {
        guard let data = object(forKey: key) as? Data else { return defaultValue }
        let object = try? NSKeyedUnarchiver.unarchivedObject(ofClass: Value.self, from: data)
    ...
    ///   - key: The key for the stored value.
    /// - Returns: The value of the specified type, or nil if it cannot be retrieved.
    public func value<Value: NSObject & NSSecureCoding>(type: Value.Type, forKey key: String) -> Value? {
        guard let data = object(forKey: key) as? Data else { return nil }
        return try? NSKeyedUnarchiver.unarchivedObject(ofClass: Value.self, from: data)
    }

    /// Saves an NSObject value conforming to NSSecureCoding to UserDefaults.
    ///
    /// If the value is nil, the data for the corresponding key is removed.
    ///
    /// - Parameters:
    ///   - value: The value to save. If nil is provided, the key's value is removed.
    ///   - key: The key under which the value is saved.
    public func setValue<Value: NSObject & NSSecureCoding>(_ value: Value?, forKey key: String) {
        guard let value = value else {
            removeObject(forKey: key)
            return
        }
